...
describe("Mypage Domain 통합 테스트", () => {
  // app 객체를 기반으로 HTTP 요청을 보낼 수 있는 agent 객체가 생성됨.
  const agent = supertest.agent(app);

  // agent 객체를 이용해서 회원가입을 하는 HTTP 요청을 보냄
  agent
  ...
  // 각 test를 하기 전에 실행함. agent에서 로그인 기능을 매 번 실행함.
  beforeEach(async () => {
    await agent
      .post("/api/auth/login")
      .send({
  ...
  worldcupData = {
    ...
    choices: [
      ...
      },
      {
        choice_name: "고양이2",
        choice_url:
          "https://img.freepik.com/free-photo/the-red-or-white-cat-i-on-white-studio_155003-13189.jpg?size=626&ext=jpg",
      },
    ],
  };
